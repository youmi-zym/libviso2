...
vector<float> read_intrinsics(string file)
{
    ...
    {
        // P_rect_101:  1.003556e+3 0.000000e+0 4.556890e+2 0.000000e+0
        //              0.000000e+0 1.003556e+3 1.976634e+2 0.000000e+0
        //              0.000000e+0 0.000000e+0 1.000000e+0 0.000000e+0
        // P = K @ T, K is intrinsic matrix, and T is translation matrix
        if(line.find("P_rect_101") != string::npos)
        {
            line.erase(std::remove(line.begin(), line.end(), '\n'), line.end());
            string values = line.substr(12);
...
int main (int argc, char** argv) {
    ...
    if (argc<2) {
        ...
        // ./kitti2015 path/to/drivingstereo/train/ half
        // ./kitti2015 path/to/drivingstereo/test/ half
        cerr << "Usage: ./drivingstereo path/to/sequence/ resolution" << endl;
        return 1;
    }

    // sequence directory
    string dir = argv[1];
    string resolution = argv[2];
    string fail_dir = dir + "pose_left/" +  resolution + "/" + "fail_record/";

    vector<string> time_sequences = glob_item_from_dir(dir + "left/");
    ...
    for (const auto& time_sequence : time_sequences) {
        ...
        string extrinsic_file = dir + "pose_left/" + resolution + "/" + time_sequence + ".txt";
        ofstream file_write(extrinsic_file);
        file_write << std::fixed;

        // read intrinsics from txt file
        string intrinsic_file = dir + "calib/" + resolution + "/" + time_sequence + ".txt";
        ...
        param.base     = 0.5443450; // baseline in meters
        ...
        vector<string> images = glob_item_from_dir(dir + "left/" + time_sequence + "/");

        bool is_first_image = true;
        for (const auto& base_name: images){

            // input file names
            string left_img_file_name  = dir + "left/" + time_sequence + "/" + base_name;
            string right_img_file_name = dir + "right/" + time_sequence + "/" + base_name;
